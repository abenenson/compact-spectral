...
lemma coe_compress_apply_of_invariant (T : E →L[𝕜] E) (V : Submodule 𝕜 E) [CompleteSpace V]
  ...
  let w : V := ⟨T (v : E), hTv⟩
  have hw : V.orthogonalProjection (T (v : E)) = w := by
    simpa [w] using (Submodule.orthogonalProjection_mem_subspace_eq_self (K := V) w)
...
lemma isCompactOperator_compress (T : E →L[𝕜] E) (hTc : IsCompactOperator (T : E → E))
  ...
  have h2 : IsCompactOperator (fun v : V => V.orthogonalProjection (T (V.subtypeL v))) :=
    h1.clm_comp V.orthogonalProjection
  simpa [compress, ContinuousLinearMap.comp_apply] using h2
...
lemma isSelfAdjoint_compress (T : E →L[𝕜] E) (hT : IsSelfAdjoint T)
  ...
  have hT' : T† = T := by
    simpa [IsSelfAdjoint] using hT
  have hAdj : (compress (T := T) (V := V))† = compress (T := T) (V := V) := by
