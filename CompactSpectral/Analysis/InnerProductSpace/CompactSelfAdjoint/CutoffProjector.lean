...
theorem finiteDimensional_largeEigenspace_of_isCompactOperator_of_isSelfAdjoint
  ...
  simpa [largeEigenspace] using
    (finiteDimensional_iSup_eigenspace_norm_ge
      (𝕜 := 𝕜) (E := E) T hT hTc hε)
...
theorem largeEigenspaceProjector_comp
  ...
  have hU_invt : U ∈ t.invtSubmodule := by
    have h_each : ∀ i : ι, t.eigenspace i.1 ∈ t.invtSubmodule := by
      intro i
      show t.eigenspace i.1 ≤ (t.eigenspace i.1).comap t
      intro x hx
  ...
  have hPTx : P (T x) = T (P x) := by
    calc
      ...
      _ = T (P x) := by
            ...
            simp
  have hPdef :
      P = largeEigenspaceProjector (𝕜 := 𝕜) (E := E) T hT hTc hε := by
    simp [P, largeEigenspaceProjector, U]
  simpa [ContinuousLinearMap.comp_apply, hPdef] using hPTx
