...
lemma tendsto_apply_of_isCompactOperator (T : E →L[𝕜] E) (hTc : IsCompactOperator (T : E → E))
  ...
  have hcont_stw : Continuous (strongToWeakDual (𝕜 := 𝕜) (E := E)) := by
    ...
    simpa [strongToWeakDual] using h2.comp h1
  have hinj : Function.Injective (strongToWeakDual (𝕜 := 𝕜) (E := E)) := by
    ...
    have h1 := (StrongDual.toWeakDual_inj (R := 𝕜) (M := E) _ _).mp hab
  ...
  have huniq :
    ...
    have htendsto :
      ...
      simpa [strongToWeakDual, WeakSpace.map_apply, toWeakSpace, toWeakSpaceCLM] using hweakDual
    have :
...
lemma continuousOn_weakClosedBall_reApplyInnerSelf_of_isCompactOperator
  ...
  have hinner :
    ...
    have hdiff :
      ...
      have hcont : Continuous fun u : E => ‖u - T (x : E)‖ := by
        simpa [sub_eq_add_neg] using
          (continuous_norm.comp (continuous_id.add (continuous_const.neg)))
      simpa using (hcont.tendsto (T (x : E))).comp hTt
    have hnorm_inner :
        Tendsto
          (fun y : WeakSpace 𝕜 E =>
    ...
    have h2 :
      ...
      have hcont :
        ...
        simpa using
          (WeakBilin.eval_continuous (B := (topDualPairing 𝕜 E).flip)
  ...
  | inl hyw_neg =>
      ...
      have hxMin_norm_eq : ‖(xMin : E)‖ = (1 : ℝ) :=
        extremizer_mem_unit_sphere (f := f)
          (fun c y => by
            show T.reApplyInnerSelf (c • (y : E)) = ‖c‖ ^ 2 * T.reApplyInnerSelf (y : E)
